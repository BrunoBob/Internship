...
function handleFileSelect(evt) {
	var f = document.getElementById('fileItem').files[0];
	var reader = new FileReader();
	reader.onload = function(e) {
		speech = reader.result
  		tab = speech.split("\n");
	}
	reader.readAsText(f);
}
...
function handleKeyboardEvent(event){
	if (event.defaultPrevented) {
  	...
  	switch (event.key) {
		case "Enter":
			if(iter > -1){
				logTime[iter] = timeSave +"@GOOD@" + iter + "@" + tab[iter] + "\n";
				if(iter < tab.length-1){
					iter++;
		...
		case "ArrowRight":
			if(iter > -1){
				logTime[iter] = timeSave +"@BAD@" + iter + "@" + tab[iter] + "\n";
				if(iter < tab.length-1){
					iter++;
					document.getElementById('toRead').innerHTML = "<strong>" + tab[iter] + "</strong>";
				}
